  ...
  async function worker() {
    while (true) {
      ...
      try {
        ...
        if (!response.ok) {
          failures += 1;
          console.error(`[detail] request ${current + 1} failed with ${response.status}`);
        ...
        failures += 1;
        console.error(`[detail] request ${current + 1} error:`, error instanceof Error ? error.message : error);
      }
    }
  }
