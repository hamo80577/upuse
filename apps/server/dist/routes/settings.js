...
export function getSettingsRoute(_req, res) {
    const s = getSettings();
...
export async function testTokensRoute(_req, res) {
    const s = getSettings();
    ...
    const branchChecks = await Promise.all(enabledBranches.map(async (branch) => {
