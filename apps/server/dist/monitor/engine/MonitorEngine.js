...
function summarizeUpstreamErrorDetail(rawDetail) {
    ...
    if (looksLikeHtmlDocument(detail)) {
        ...
        const isCloudflareTunnel = /cloudflare/i.test(detail) ||
...
export class MonitorEngine {
    ...
    isLifecycleCurrent(expectedLifecycleId) {
    ...
    setSourceError(source, context, e) {
    ...
    resolveThresholds(branch, settings) {
        return resolveBranchThresholdProfile(branch, settings);
    }
    inferMonitorCloseReason(branch, metrics, settings) {
    ...
    hasTrustedMonitorRuntime(runtime, settings) {
        if (!runtime)
            return false;
    ...
    isTrackedUpuseClosure(runtime, closedUntil) {
        if (!closedUntil)
            return false;
    ...
    matchesExpectedMonitorCloseWindow(runtime, closedUntil) {
        if (!closedUntil || !runtime?.lastUpuseCloseAt)
            return false;
        const settings = getSettings();
        ...
        if (!lastCloseAt.isValid || !actualUntil.isValid)
            return false;
    ...
    isMonitorOwnedClosure(runtime, availability) {
        if (!availability || availability.availabilityState !== "CLOSED_UNTIL")
            return false;
        const settings = getSettings();
        if (!this.hasTrustedMonitorRuntime(runtime, settings))
            return false;
        return (this.isTrackedUpuseClosure(runtime, availability.closedUntil) ||
            this.matchesExpectedMonitorCloseWindow(runtime, availability.closedUntil));
    }
    hasActiveTrackedMonitorWindow(runtime, nowIso) {
        const settings = getSettings();
        if (!this.hasTrustedMonitorRuntime(runtime, settings))
            return false;
        ...
        if (!now.isValid || !lastUntil.isValid)
            return false;
        return now <= lastUntil.plus({ seconds: 120 });
    }
    inferCloseStartedAt(closedUntil, durationMinutes) {
        if (!closedUntil)
            return undefined;
    ...
    inferObservedExternalCloseStartedAt(runtime, closedUntil) {
        if (!runtime?.lastExternalCloseAt || !closedUntil)
            return undefined;
        if (runtime.lastExternalCloseUntil !== closedUntil)
            return undefined;
    ...
    buildClearedTrackedRuntimePatch(runtime) {
        const patch = {};
        if (runtime?.lastUpuseCloseUntil)
            patch.lastUpuseCloseUntil = null;
        if (runtime?.lastUpuseCloseReason)
            patch.lastUpuseCloseReason = null;
        if (runtime?.lastUpuseCloseAt)
            patch.lastUpuseCloseAt = null;
        if (runtime?.lastUpuseCloseEventId)
            patch.lastUpuseCloseEventId = null;
        if (runtime?.externalOpenDetectedAt)
            patch.externalOpenDetectedAt = null;
        return patch;
    }
    syncTrackedMonitorRuntime(branch, metrics, runtime, closedUntil, settings) {
        const patch = {};
        ...
        return getRuntime(branch.id);
    }
    extractClosedUntilCandidate(payload) {
        const candidates = [
            payload?.closedUntil,
    ...
    syncExternalClosureState(nowIso) {
        ...
        for (const branch of branches) {
            ...
            const ownedByUpuse = this.isMonitorOwnedClosure(runtime, availability) ||
                Boolean(availability.availabilityState === "CLOSED_UNTIL" &&
                    availability.closedUntil &&
                    monitorWindowStillActive &&
                    !runtime?.externalOpenDetectedAt);
            const isExternalTempClose = Boolean(availability.availabilityState === "CLOSED_UNTIL" && availability.closedUntil && !ownedByUpuse);
            const isMonitorWindowAlias = Boolean(availability.availabilityState === "CLOSED_UNTIL" &&
                availability.closedUntil &&
                monitorWindowStillActive &&
                !runtime?.externalOpenDetectedAt);
            if (availability.availabilityState === "CLOSED_UNTIL" &&
                ...
                if (runtime?.lastExternalCloseUntil || runtime?.lastExternalCloseAt) {
                    ...
                    runtime = getRuntime(branch.id);
                }
            }
            if (isExternalTempClose) {
                const externalClosedUntil = availability.closedUntil;
                const shouldPersistExternalWindow = Boolean(externalClosedUntil &&
                ...
                if (externalClosedUntil && (shouldPersistExternalWindow || shouldResetTrackedRuntime)) {
                    runtime = setRuntime(branch.id, {
                        ...
                        lastExternalCloseAt: runtime?.lastExternalCloseUntil === externalClosedUntil && runtime?.lastExternalCloseAt
                    ...
                    if (shouldPersistExternalWindow) {
                        log(branch.id, "WARN", untilLabel ? `TEMP CLOSE — external source until ${untilLabel}` : "TEMP CLOSE — external source");
                    }
                }
                continue;
            }
    ...
    getSnapshot() {
        ...
        const branchSnapshots = monitoredBranches.map((b) => {
            ...
            const preparation = this.currentPreparation(this.preparationByVendor.get(b.ordersVendorId) ?? {
    ...
    async runOrdersCycle(options, expectedLifecycleId) {
        if (!this.isLifecycleActive(expectedLifecycleId))
            return;
    ...
    async runAvailabilityCycle(options, expectedLifecycleId) {
        if (!this.isLifecycleActive(expectedLifecycleId))
            return;
        await this.jobMutex.runExclusive(async () => {
            if (!this.isLifecycleActive(expectedLifecycleId))
    ...
    async reconcile(trigger, expectedLifecycleId) {
        ...
        for (const branch of branches) {
            ...
            let runtime = getRuntime(branch.id);
            if (avCached?.availabilityState === "CLOSED_UNTIL" &&
            ...
            if (avCached?.availabilityState === "OPEN" &&
            ...
            await this.actionMutex.runExclusive(async () => {
                ...
                if (decision.type === "CLOSE") {
                    if (current.availabilityState !== "OPEN")
                        return;
                    if (!this.isLifecycleActive(expectedLifecycleId))
                        return;
                    ...
                    log(branch.id, "INFO", isReappliedAfterExternalOpen
                        ? confirmedUntilLabel
                ...
                if (decision.type === "EARLY_OPEN") {
                    if (current.availabilityState !== "CLOSED_UNTIL")
                        return;
                    if (!this.isLifecycleActive(expectedLifecycleId))
                        return;
                    let rt = getRuntime(branch.id);
