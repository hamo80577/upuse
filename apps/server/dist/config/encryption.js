...
function deriveCipherKey(secret) {
    return crypto.createHash("sha256").update(secret).digest();
}
export function deriveEncryptionKeyId(secret) {
    return crypto.createHash("sha256").update(secret).digest("hex").slice(0, KEY_ID_LENGTH);
}
export function parseEncryptionSecretList(raw) {
    if (!raw)
        return [];
    return raw
        .split(/[\r\n,]+/)
...
export function createEncryptionKeyring(primarySecret, fallbackSecrets = []) {
...
function parseVersionedPayload(payload) {
...
function orderDecryptionKeys(keys, keyId) {
...
export function createCryptoBox(keyring) {
    const decryptWithMetadata = (payload) => {
        ...
        throw new Error(parsedPayload
            ...
            : "Unable to decrypt legacy encrypted settings with the configured keyring.", { cause: lastError instanceof Error ? lastError : undefined });
    };
    const encrypt = (plain) => {
    ...
    return {
        ...
        assertCanDecryptAll(payloads) {
            for (const payload of payloads) {
                if (!this.canDecrypt(payload)) {
                    throw new Error("Configured UPUSE_SECRET values cannot decrypt the existing stored settings. Restore the previous secret or include it in UPUSE_SECRET_PREVIOUS.");
                }
            }
        },
    };
