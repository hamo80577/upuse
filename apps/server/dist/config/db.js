...
function resolveBootstrapAdmin(env) {
    ...
    if (!hasAnyValue)
        return null;
    if (!email || !password) {
        throw new Error("UPUSE_BOOTSTRAP_ADMIN_EMAIL and UPUSE_BOOTSTRAP_ADMIN_PASSWORD must both be set when bootstrapping the first admin user.");
    }
...
function maybeSeedBootstrapAdmin() {
    ...
    if (!bootstrapAdmin) {
        if (!usersCountRow.count) {
            const message = "No application users exist. Set UPUSE_BOOTSTRAP_ADMIN_EMAIL and UPUSE_BOOTSTRAP_ADMIN_PASSWORD to create the first admin account.";
            if (isProduction()) {
    ...
    VALUES (?, ?, ?, ?, 1, ?)
  `).run(bootstrapAdmin.email, bootstrapAdmin.name, bootstrapAdmin.role, hashPassword(bootstrapAdmin.password), new Date().toISOString());
...
function migrateLegacyUserRoles() {
    ...
    const hasUnsupportedRoles = Boolean(db.prepare("SELECT 1 FROM users WHERE LOWER(TRIM(role)) NOT IN ('admin', 'user') LIMIT 1").get());
    if (!hasLegacyConstraint && !hasUnsupportedRoles)
        return;
