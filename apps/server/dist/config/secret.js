...
export function resolveEncryptionSecret(options) {
    ...
    if (persistedSecret) {
        if (encryptedSettings.length > 0 && !canDecryptAllWithSecret(encryptedSettings, persistedSecret)) {
            throw new Error(`${secretFilePath} exists but cannot decrypt the current stored tokens. Restore the original UPUSE_SECRET or replace ${DEV_SECRET_FILE_NAME} with the correct secret.`);
        }
        warn(`WARNING: UPUSE_SECRET is not set. Using the persisted development key at ${secretFilePath}. Configure UPUSE_SECRET to silence this warning.`);
    ...
    if (encryptedSettings.length > 0) {
        if (canDecryptAllWithSecret(encryptedSettings, LEGACY_DEV_SECRET)) {
            ...
            warn(`WARNING: UPUSE_SECRET is not set. Adopted the legacy development key and persisted it to ${secretFilePath} for compatibility. Set UPUSE_SECRET to use an explicit key.`);
            return LEGACY_DEV_SECRET;
        }
        throw new Error(`UPUSE_SECRET is missing and no ${DEV_SECRET_FILE_NAME} file exists. Existing encrypted settings cannot be safely decrypted without the original key. Restore UPUSE_SECRET or create ${secretFilePath} with the previous secret.`);
    }
    ...
    warn(`WARNING: UPUSE_SECRET is not set. Generated a development-only key at ${secretFilePath}. This keeps localhost usable, but production must always set UPUSE_SECRET explicitly.`);
    return generatedSecret;
}
function readPersistedDevSecret(secretFilePath, fileSystem) {
