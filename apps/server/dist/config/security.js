...
function parseBoundedInteger(raw, fallback, options) {
...
export function resolveSecurityConfig(env = process.env) {
    return {
        ...
        maxStreamConnectionsPerUser: parseBoundedInteger(env.UPUSE_STREAM_MAX_CONNECTIONS_PER_USER, DEFAULT_MAX_STREAM_CONNECTIONS_PER_USER, {
            min: 1,
            max: 20,
        }),
        maxStreamConnectionsTotal: parseBoundedInteger(env.UPUSE_STREAM_MAX_CONNECTIONS_TOTAL, DEFAULT_MAX_STREAM_CONNECTIONS_TOTAL, {
            min: 1,
            max: 1_000,
        }),
    };
