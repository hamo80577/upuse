...
export function hasFullUpuseAccess(user: AppUser | null | undefined) {
  return hasUpuseAccess(user) && user?.role !== "tracker";
}
...
export function requireFullUpuseAccess(): RequestHandler {
  return (req, res, next) => {
    if (hasFullUpuseAccess(req.authUser)) {
      next();
      return;
    }
