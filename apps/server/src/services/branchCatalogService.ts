...
import {
  getBranchCatalogItem,
  getBranchCatalogSyncState,
  listBranchCatalog,
  markBranchCatalogMissing,
  setBranchCatalogSyncState,
  updateBranchCatalogResolution,
  upsertBranchCatalogSources,
} from "./branchCatalogStore.js";
import type { BranchCatalogItem, BranchCatalogResponse, BranchCatalogSyncState } from "../types/models.js";
...
interface WarehouseLookupItem {
  id: number;
  name?: string | null;
  platformVendorId?: string[] | string | null;
  externalId?: string[] | string | null;
}

function normalizeWarehouseIds(value: WarehouseLookupItem["platformVendorId"] | WarehouseLookupItem["externalId"]): string[] {
...
function matchesAvailabilityVendorId(item: WarehouseLookupItem, availabilityVendorId: string) {
...
function extractWarehouseLookupItems(payload: unknown): WarehouseLookupItem[] {
  if (Array.isArray(payload)) return payload as WarehouseLookupItem[];
  if (Array.isArray((payload as any)?.data)) return (payload as any).data as WarehouseLookupItem[];
  return [];
}

function describeSyncError(error: unknown) {
  const status = (error as any)?.response?.status;
  const upstreamMessage = (error as any)?.response?.data?.message;
  const fallback = (error as any)?.message || "Catalog sync failed";
...
function resolveSyncState(globalEntityId: string): { syncState: BranchCatalogSyncState; lastSyncedAt: string | null; lastError: string | null } {
...
async function resolveCatalogRows(globalEntityId: string, ordersToken: string, resolvedAt: string) {
  ...
  if (!rows.length) return { hasFatalError: false, lastError: null as string | null };
...
async function runCatalogSync(globalEntityId: string) {
...
export function triggerBranchCatalogRefresh(globalEntityId: string, options?: { force?: boolean }) {
...
export async function refreshBranchCatalogNow(globalEntityId: string) {
...
export function getBranchCatalogResponse(globalEntityId: string, options?: { triggerRefreshIfStale?: boolean }): BranchCatalogResponse {
  const rows = listBranchCatalog(globalEntityId);
  const branches = listBranches();
  const mappedByAvailabilityVendorId = new Map(branches.map((branch) => [branch.availabilityVendorId, branch] as const));
  ...
  const items: BranchCatalogItem[] = rows.map((row) => {
...
export function getResolvedCatalogBranchForAdd(globalEntityId: string, availabilityVendorId: string) {
  const item = getBranchCatalogItem(availabilityVendorId);
  if (!item || item.globalEntityId !== globalEntityId) return null;
  return item;
}
