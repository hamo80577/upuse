...
interface SettingsTokenTestJobRow {
  id: string;
  status: SettingsTokenTestSnapshot["status"];
  createdAt: string;
  ...
  ordersConfigured: number;
  ordersConfigValid: number;
  ordersConfigMessage: string | null;
  ordersProbeOk: number;
  ordersProbeStatus: number | null;
  ordersProbeMessage: string | null;
...
interface SettingsTokenTestResultRow {
  branchId: number;
  name: string;
...
function jobRow(jobId: string) {
  return db.prepare<[string], SettingsTokenTestJobRow>("SELECT * FROM settings_token_test_jobs WHERE id = ?").get(jobId) ?? null;
}

function resultRows(jobId: string) {
  return db.prepare<[string], SettingsTokenTestResultRow>(`
...
function upsertJob(jobId: string, patch: Partial<SettingsTokenTestJobRow>) {
...
export function getSettingsTokenTestSnapshot(jobId: string): SettingsTokenTestSnapshot | null {
  const row = jobRow(jobId);
  if (!row) return null;

  const branches: OrdersTokenBranchTestResult[] = resultRows(jobId).map((item) => ({
...
function insertBranchResult(jobId: string, branch: ResolvedBranchMapping, result: {
  ok: boolean;
  status: number | null;
  message?: string;
  sampleVendorName?: string | null;
  ...
  `).run(
    ...
    branch.id,
    branch.name,
    branch.ordersVendorId,
    result.ok ? 1 : 0,
    result.status,
    result.message ?? null,
    result.sampleVendorName ?? null,
    processedAt,
  );
...
async function runSettingsTokenTestJob(jobId: string, settings: Settings, branches: ResolvedBranchMapping[]) {
  upsertJob(jobId, {
    status: "running",
...
export function startSettingsTokenTestJob() {
  ...
  void runSettingsTokenTestJob(jobId, settings, branches).catch((error: any) => {
    const normalized = normalizeError(error);
    upsertJob(jobId, {
      status: "failed",
      completedAt: new Date().toISOString(),
